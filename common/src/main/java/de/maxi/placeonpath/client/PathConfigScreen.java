...
public class PathConfigScreen extends Screen {
    ...
    public PathConfigScreen(Screen parent) {
        super(new TranslatableComponent("placeonpath.config.title"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        this.search = new EditBox(this.font, this.width / 2 - boxW / 2, 28, boxW, 18,
                new TranslatableComponent("placeonpath.config.search"));
        this.search.setResponder(s -> { this.query = s.toLowerCase(); this.scroll = 0; });
        addButton(this.search);

        int by = this.height - 28;
        addButton(new Button(this.width / 2 - 154, by, 150, 20,
                new TranslatableComponent("placeonpath.config.save_exit"), b -> saveAndExit()));
        addButton(new Button(this.width / 2 + 4, by, 150, 20,
                new TranslatableComponent("placeonpath.config.exit"), b -> exitWithoutSave()));
    ...
    public void render(PoseStack g, int mouseX, int mouseY, float partialTick) {
        ...
        for (LayoutItem item : items) {
            if (item instanceof HeaderItem) {
                ...
                renderHeader(g, h, sy);
            } else if (item instanceof CellItem) {
                CellItem c = (CellItem) item;
    ...
    /** 1.18.2 has no GuiComponent scissor helper; translate GUI coords to framebuffer pixels. */
    private void enableScissor(int x1, int y1, int x2, int y2) {
        double scale = this.minecraft.getWindow().getGuiScale();
        int fbH = this.minecraft.getWindow().getHeight();
        RenderSystem.enableScissor((int) (x1 * scale), (int) (fbH - y2 * scale),
                (int) ((x2 - x1) * scale), (int) ((y2 - y1) * scale));
    }

    private void drawLegend(PoseStack g) {
        Component keep = new TranslatableComponent("placeonpath.config.legend_keep");
        Component dirt = new TranslatableComponent("placeonpath.config.legend_dirt");
        int gap = 18;
    ...
    private void renderHeader(PoseStack g, HeaderItem h, int sy) {
        ...
        Component name = new TranslatableComponent(h.cat.langKey());
    ...
    private void renderCell(PoseStack g, CellItem c, int sy, int mouseX, int mouseY) {
        ...
        this.minecraft.getItemRenderer().renderGuiItem(new ItemStack(c.entry.block), c.x + 2, sy + (c.h - 16) / 2);
        int textX = c.x + 22;
    ...
    private void exitWithoutSave() {
        ...
        }, new TranslatableComponent("placeonpath.config.discard_title"),
           new TranslatableComponent("placeonpath.config.discard_message")));
    }
